...
class ClientboundWebsocketHandshakeHandler(uri: URI) : SimpleChannelInboundHandler<FullHttpResponse>() {
    ...
    override fun channelActive(ctx: ChannelHandlerContext) {
        Logging.debug("[Handshake] Starting handshake")
    ...
    override fun channelRead0(ctx: ChannelHandlerContext, msg: FullHttpResponse) {
        ...
        try {
            ...
            Logging.debug("[Handshake] Handshake complete, switching to frame handling phase")
            switchToResetPhase(ctx.pipeline())
        } catch (e: WebSocketHandshakeException) {
            Logging.debug("[Handshake] WebSocket Client failed to connect")
            ch.close()
        }
    }

    private fun switchToResetPhase(ctx: ChannelPipeline) {
        ctx.remove(this)
        ctx.addLast(ServerboundPacketEncoder())
        ctx.addLast(ClientboundPacketDecoder())
        ctx.addLast(ClientboundResetHandler())
    }
}